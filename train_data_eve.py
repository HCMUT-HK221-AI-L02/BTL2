            ...
            if turn == side:
                ...
                if turn == 1:
                    if remain_move["remain_move_o"] >= 40:
                        appendData(FILENAME_OPENING, new_data)
                    elif remain_move["remain_move_o"] >= 20:
                        appendData(FILENAME_MIDDLE, new_data)
                    else:
                        appendData(FILENAME_LATE, new_data)
                else:
                ...
                if master_board.advantageTeam() != 0 and master_board.advantageTeam() != side:
                    appendData(FILENAME_STRUGGLE, new_data)

                (O, X) = master_board.countPieceByTeam()
                if side == 1 and O >= int(0.75*16):
                    appendData(FILENAME_CLOSEOUT, new_data)
